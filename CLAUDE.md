...
```bash
composer dev           # Start dev server, queue worker, log viewer, and Vite concurrently
composer test          # Run full Pest test suite
composer fix           # Fix code style with Pint
composer lint          # Check code style (no write)
composer analyse       # Run PHPStan (level 8)
composer prepare-commit  # fix + analyse + test (run before committing)
```

Single test:
```bash
vendor/bin/pest tests/Feature/Customers/IndexTest.php
...
**Timatic is a time-tracking system** for engineers to log work against customers, tickets, and budgets. There are two interfaces: a Filament admin panel (subdomain `admin.*`) and a JSON:API (subdomain `api.*`).

### Domain model

```
Customer
  └── Budget (immutable fields: type, customer, frequency, start)
        └── BudgetVersion (mutable: title, price, hours — audit trail)
              └── Period (dynamically generated, never stored)
                    └── Entry (time log: ticket, minutes, budget or pay-per-hour)
                          └── Overtime (night/evening/weekend buckets, stored as JSON)

Source (integration) → Event → Activity (grouped) → EntrySuggestion → Entry
```

**Periods are not stored** — they are calculated at runtime from `Budget.renewal_frequency` and start/end dates.

**Budget versioning** — `Budget` holds immutable fields; `BudgetVersion` holds mutable fields so history is preserved without data loss.

**Entry without budget** (`budget_id = null`, `is_internal = false`) is treated as "paid per hour".

### Integration system

All third-party integrations (Jira, Bitbucket, …) are path-based Composer packages under `/integrations/{name}/`. Each package:

1. Has a `ServiceProvider` that registers in two registries:
   - `IntegrationTypeRegistry` — maps type string to Filament UI pages
   - `TicketProviderRegistry` — maps type string to `TicketProviderInterface` implementation
2. Implements OAuth 2.0 via a `{Name}OAuthService` (tokens stored encrypted in `Integration.config`)
...
- Built on `timacdonald/json-api` (custom fork)
- Controllers use `Spatie\QueryBuilder` for filtering/sorting/includes
- All API routes live in `routes/web.php` (not `api.php`) with `middleware(['api', 'auth:api,web'])`
- Requests are wrapped via `EncapsulateRequestBodyWithData` middleware
- Auth: Bearer token or API key (custom guard in `AppServiceProvider`)
- OpenAPI docs at `/docs` via Scramble

### Filament admin

- Panel config in `app/Providers/Filament/AdminPanelProvider.php`
- Shield (`bezhansalleh/filament-shield`) for role/permission management
- `IntegrationResource` dynamically routes to integration-specific pages via `IntegrationTypeRegistry`

### Testing

- Pest (PHPUnit wrapper), 3 suites: `tests/Feature/`, `tests/Integration/`, `tests/Unit/`
- Tests use `RefreshDatabase` and a custom `LoginUser` trait
- PHPStan at level 8 with Larastan; config in `phpstan.neon`
