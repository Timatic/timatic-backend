...
namespace Timatic\Nmbrs\Http\Controllers;
    ...
    public function __invoke(Request $request, OAuthService $oauth): RedirectResponse
    {
        ...
        if ($request->has('error')) {
            return $this->redirectToSettings($integrationId)
                ->with('nmbrs_error', 'NMBRS verbinding geweigerd: '.$request->string('error_description'));
        }
        ...
        try {
            ...
            return redirect(SettingsPage::getUrl(['record' => $integration->getKey()]))
                ->with('nmbrs_success', 'NMBRS verbinding succesvol.');
        } catch (\Throwable $e) {
            return $this->redirectToSettings($integrationId)
                ->with('nmbrs_error', 'Verbinding mislukt: '.$e->getMessage());
        }
    }
